 ...
 * 
 * USO:
 *   npm run create-admin -- --username admin --password tu_contraseña_segura
 * 
 * O con variables de entorno:
 *   ADMIN_USERNAME=admin ADMIN_PASSWORD=tu_contraseña npm run create-admin
 * 
 * SEGURIDAD:
...
async function createAdmin() {
  ...
  if (!username || !password) {
    ...
    console.error('');
    console.error('Uso:');
    console.error('  npm run create-admin -- --username admin --password tu_contraseña');
    console.error('');
    console.error('O con variables de entorno:');
    console.error('  ADMIN_USERNAME=admin ADMIN_PASSWORD=contraseña npm run create-admin');
    process.exit(1);
  }
